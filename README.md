...
- 후기 체크박스로 여러 대학·학과의 후기를 선택하면 AI 모의면접 카드 활성화
- 학생부 면접 / 제시문 면접 / 기본인성 면접
- 면접 시작 전 생기부 유무 확인: 있으면 생기부와 사례집을 함께 활용하고, 없으면 기존처럼 사례집으로 진행
- **체크한 후기만** 모의면접과 인쇄에 사용. 여러 대학·학과를 함께 선택할 수 있으며 검색 조건·페이지 변경 후에도 선택 유지
- 공백 차이를 제거한 동일 질문 중복 제외, 질문 수와 선택 대학·학과 표시
- 선택 후기 목록과 질문별 대학·학과·학년도 출처를 포함한 한국어 프롬프트 생성. 여러 대학·학과를 구분하고 균형 있게 연습하도록 안내
...
검색창은 후기 목록을 좁히는 용도입니다. 검색하지 않거나 명칭을 일부만 입력해도 후기 체크박스로 모의면접 대상을 선택할 수 있습니다. 체크한 후기만 프롬프트에 포함되며 검색·페이지 변경으로 선택이 사라지지 않습니다. 검색 결과 전체 선택은 현재 필터에 맞는 모든 페이지의 후기를 추가하고, 해제하면 해당 검색 결과만 제외합니다. ‘선택 해제’는 숨겨진 후기를 포함해 전체 선택을 비웁니다. 선택 후기가 바뀌면 이전 복사 안내와 미리보기가 초기화됩니다. 질문이 없거나 자료 로딩에 실패하면 시작 버튼은 비활성화됩니다. 프롬프트를 임의로 자르지 않으며, 너무 길면 선택 후기를 줄이세요.
...
검증 범위: 실제 클립보드 내용과 미리보기의 일치, ChatGPT 대상 URL과 opener 제거, 복사 거절, 팝업 차단, 수동 선택, 질문 없음, 세 가지 프롬프트, 체크한 후기만 사용·복수 대학/학과 선택·중복 질문 출처 보존, 후기 조회·인쇄 스타일·페이지 이동, 모바일 가로 넘침, 데이터 로딩 실패·재시도.
...
5. Preview Deployment의 HTTPS 주소에서 후기 복수 선택 → 미리보기 → 복사 → ChatGPT 열기 → 붙여넣기를 확인합니다. 팝업 차단 상태와 모바일 Safari도 확인합니다.
...
초기 데이터 파일은 10,774,311바이트에서 559,931바이트로 약 94.8% 줄었습니다(압축 전 크기, 로딩 시간 측정값은 아님). 질문 개수는 빌드 시 계산하고 본문은 상세 보기, 체크한 후기의 모의면접, 인쇄 선택 시 필요한 학과 자료만 가져옵니다. 동일 본문 요청은 공유하며 실패하면 재시도할 수 있습니다. 인쇄 버튼은 선택 자료가 모두 준비되면 활성화됩니다.
