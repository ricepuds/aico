...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch("/data/index.json", { signal: controller.signal }).then(async response => {
      if (!response.ok) throw new Error("질문 자료를 불러오지 못했습니다.");
      const data: unknown = await response.json();
      if (!Array.isArray(data) || data.some(record => !record || ["id", "year", "uni", "dept", "content", "region", "field", "interview"].some(key => typeof record[key] !== "string") || !Number.isInteger(record.questionCount) || typeof record.dataFile !== "string" || !/^details\/[a-f0-9]{24}\.json$/.test(record.dataFile))) throw new Error("질문 자료 형식을 확인해 주세요.");
  ...
  const fields = useMemo(() => unique(cases.map(record => record.field)), [cases]);
  const interviews = useMemo(() => unique(cases.map(record => record.interview)), [cases]);
  const resultUniversities = useMemo(() => new Set(filtered.map(record => record.uni)).size, [filtered]);
  const resultDepartments = useMemo(() => new Set(filtered.map(record => record.dept)).size, [filtered]);
  ...
  return <>
    ...
    <main className="page-shell no-print">
      ...
      <section className="search-panel" aria-labelledby="search-title"><div className="section-top"><h2 id="search-title"><span className="step-number">01</span> 지원 대학 · 학과 찾기</h2><button className="text-button" onClick={() => { setFilters(initialFilters); setPage(1); }}>검색 초기화 ↺</button></div>
        ...
        <div className="extra-filters"><label>권역<select value={filters.region} onChange={event => change("region", event.target.value)}><option value="">전체 권역</option>{regions.map(value => <option key={value}>{value}</option>)}</select></label><label>계열<select value={filters.field} onChange={event => change("field", event.target.value)}><option value="">전체 계열</option>{fields.map(value => <option key={value}>{value}</option>)}</select></label><label>자료의 면접유형<select value={filters.interview} onChange={event => change("interview", event.target.value)}><option value="">전체 유형</option>{interviews.map(value => <option key={value} value={value}>{interviewLabel(value)}</option>)}</select></label></div>
      </section>
